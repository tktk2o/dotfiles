...
|---|---|---|
| `twr` | 過去の tmux ウィンドウを resurrect 履歴から復元する（fzf） | `setup.sh:203` |
| `csr` | 過去の Claude セッションを自分の発言で検索して再開する（fzf） | `setup.sh:404` |
| `keys` | この dotfiles が提供するコマンド / キーバインドを一覧・検索する（fzf） | `setup.sh:425` |
| `nvim-open` | ファイルを nvim（peek プロファイル）で開く。Finder / ブラウザからの入口 | `setup.sh:480` |
